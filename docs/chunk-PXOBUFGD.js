...
var STORAGE_KEY_CACHE_REQUESTS = "cache_requests";
var STORAGE_KEY_CACHE_MAX_AGE = "cache_max_age";

// src/app/shared/helpers/storage.helpers.ts
var fromStorage = (key, defaultValue, next = tap(() => {
})) => fromEvent(window, "storage").pipe(filter((event) => event.key === key), map((event) => JSON.parse(event.newValue) ?? defaultValue), next, startWith(JSON.parse(localStorage.getItem(key)) ?? []));
...
var LocationService = class LocationService2 {
  constructor() {
    this.storageEvents$ = fromStorage(STORAGE_KEY_LOCATIONS, [], delay(1e3));
    this.storageEvents = toSignal(this.storageEvents$);
    this.locationsSignal = linkedSignal(() => this.storageEvents());
    this.locations = this.locationsSignal.asReadonly();
    this.locations$ = toObservable(this.locations);
  }
  /**
...
// src/app/weather/services/location-weather.service.ts
...
var LocationWeatherService = class LocationWeatherService2 {
  constructor() {
    this.locationService = inject(LocationService);
    this.weatherService = inject(WeatherService);
    this.currentConditions$ = this.locationService.locations$.pipe(
      switchMap((locations) => {
        return forkJoin(locations.map((location) => {
          return this.weatherService.getCurrentConditionsWithIcon(location).pipe(map((currentConditions) => ({
      ...
      map((currentConditions) => currentConditions.filter((cc) => cc !== null))
    );
    this.getCurrentConditions = toSignal(this.currentConditions$);
  }
};
LocationWeatherService = __decorate2([
  Injectable()
