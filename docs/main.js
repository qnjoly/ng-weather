...
var VERSION = new Version2("19.2.0");
...
var app_component_default = '<p>Current Max Age: {{ maxAge() | prettyMs }}</p>\n<form [formGroup]="form" (ngSubmit)="onSubmit()">\n  <label for="maxAge">Max Age</label>\n  <input type="number" id="maxAge" name="maxAge" formControlName="maxAge" />\n  <button type="submit">Submit</button>\n</form>\n<hr />\n<router-outlet></router-outlet>\n';
...
var CONF_CACHE = new InjectionToken("CONF_CACHE");
var RequestCacheService = class RequestCacheService2 {
  constructor() {
    this.conf = inject(CONF_CACHE);
    this.storageEvents$ = fromStorage(STORAGE_KEY_CACHE_REQUESTS, {});
    this.storageEvents = toSignal(this.storageEvents$);
    this.cache = linkedSignal(() => this.storageEvents());
    this.cache$ = toObservable(this.cache).pipe(shareReplay(1));
    this.maxAgeEvents$ = fromStorage(STORAGE_KEY_CACHE_MAX_AGE, this.conf.maxAge.toString());
    this.maxAgeEvents = toSignal(this.maxAgeEvents$.pipe(
    ...
    this.maxAge = linkedSignal(() => this.maxAgeEvents());
    this.getMaxAge = this.maxAge.asReadonly();
  }
  get(req, type) {
    const isObservable = type === "observable";
    const cached = this.cache()[req.urlWithParams];
    if (!cached) {
      return isObservable ? of(void 0) : void 0;
    }
    if (this.isExpired(cached)) {
      return isObservable ? of(void 0) : void 0;
    }
    if (isObservable && cached.inProgress) {
      return this.cache$.pipe(map((c) => c[req.urlWithParams]), filter((c) => !c.inProgress), map((c) => c?.response), take(1));
    }
...
var AppComponent = class AppComponent2 {
  constructor() {
    this.cacheRequestService = inject(RequestCacheService);
    this.fb = inject(FormBuilder);
    this.maxAge = this.cacheRequestService.getMaxAge;
...
// src/app/shared/interceptors/cache-api.interceptor.ts
...
var CacheApiInterceptor = class CacheApiInterceptor2 {
  constructor() {
    this.requestCacheService = inject(RequestCacheService);
  }
  /**
  ...
  intercept(req, next) {
    ...
    return this.requestCacheService.get(req, "observable").pipe(switchMap((cachedResponse) => cachedResponse ? of(new HttpResponse(cachedResponse)) : this.requestCacheService.cacheRequest(req, next)));
  }
  /**
  ...
  isCacheable(req) {
    if (req.method !== "GET")
      return { statut: false, motif: "METHOD" };
    return { statut: true };
  }
};
CacheApiInterceptor = __decorate3([
  Injectable({ providedIn: "root" })
], CacheApiInterceptor);

// src/app/shared/pipes/pretty-ms.pipe.ts
...
var PrettyMsPipe = class PrettyMsPipe2 {
  /**
